...
2. 角色分配规则（关键）：
   - 建议角色从以下选择：产品、前端、后端、测试
   - 每个功能应分配所有合理涉及的角色的，不要只分配单个角色
   - 大部分功能至少需要 2 个角色（例如前端功能需 前端+产品，后端功能需 后端+产品）
   - 前后端都涉及的功能需 前端+后端+产品
   - 产品角色通常参与所有功能的需求确认
   - 测试角色在复杂功能(weight≥3)时加入
...
7. 每个需求必须按「模块-功能-子功能」三级粒度拆分，subfeature 不能为空，至少要细化到子功能级别。例如"客户管理-文档管理-上传下载"、"客户管理-文档管理-权限控制"。"""
